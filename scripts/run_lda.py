...
import shutil
import random
from scripts.text2ldac import gen_ldac_corpus
...
def divide_corpus(data_dir, lda_model_dir):
    ...
    with codecs.open("%s/corpus_train.dat" % lda_model_dir, 'w', 'utf-8') as datfile:
        datfile.write(str(len(train)) + '\n')
        for doc in train:
            datfile.write(doc + '\n')
    with codecs.open("%s/corpus_test.dat" % lda_model_dir, 'w', 'utf-8') as testfile:
        testfile.write(str(len(test)) + '\n')
        for doc in test:
            testfile.write(doc + '\n')
    print("Training documents: ", len(train))
    print("Testing documents: ", len(test))
    print("Saved training files to [%s/corpus_train.dat]" % lda_model_dir)
    print("Saved testing files to [%s/corpus_test.dat]\n" % lda_model_dir)
...
if __name__ == '__main__':
    ...
    data_dir = "./data_cleaned/final_out"
    lda_dir = "./models/lda"
    db_dir = "./models/db"
